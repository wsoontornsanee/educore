...
5. **Row Lock Order for Money Paths (wallet, then sale, then dispute):**
   ...
   - Isolation is READ COMMITTED (Django's MySQL default; `educore/settings` sets no `isolation_level`; check with `SELECT @@transaction_isolation`). InnoDB takes gap locks at that level only for foreign-key and duplicate-key checks, so a `select_for_update` on a key that does not exist yet does not by itself block other inserts (a 30-round scratch test with two such charges found no deadlock), and a plain read after waiting for a lock already sees the latest committed row. State the isolation level in any concurrency reproduction: a REPEATABLE READ scratch test produced a deadlock the app cannot have.
   - Re-check "already done?" (idempotency key, dispute status, settlement status) with a locking read once the lock is held. It is cheap, needs no reasoning about snapshots, and stays correct if the isolation level ever changes to REPEATABLE READ, where a plain read after a lock wait would show the transaction's older snapshot. `record_wallet_transaction` and `charge_qr_session` do this.
