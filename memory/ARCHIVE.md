...
## Frontend Track: Milestones M0–M5 and Mobile Track (PRs #77–#82, #85)
- **Date:** 2026-09-16
- **Milestone:** Frontend Delivery Plan (Web HTMX/Tailwind & React Native Expo Mobile) 100% Complete & Merged
- **Details:**
  - **Milestone 0 (PR #77):** Shared web foundation (`frontend/` tree, `templates/base.html`, `templates/components/`), Tailwind CSS tokens (`#C8102E`, 0px border radius, Indonesian typography Jakarta Sans + IBM Plex), DRF `IdempotentViewMixin` backed by `core.IdempotencyRecord`, component primitives (`StatusBadge`, `StaleOfflineBanner`, cursor-paginated list partial), and `id-ID` translation strings.
  - **Milestone 1 (PR #78):** Homework Completion Bar (`ACD-030`) — 4 mutually exclusive segments (`graded`, `submitted`, `late`, `missing`), invariant `sum == total`, reminder status read endpoint (`GET /homework/:id/remind-status/`), and responsive HTMX component (`homework_completion_bar.html`).
  - **Milestone 2 (PR #79):** Teacher Gradebook Grid & Merge Prompt (`ACD-006`, `TCH-005..007`) — Spreadsheet keyboard navigation (arrows/Tab/Enter/Esc), 4 visual cell states, optimistic concurrency 409 merge prompt modal (`expected_version` conflict resolution), and 3-layer tenancy 404 enforcement.
  - **Milestone 3 (PR #80):** Homework Grading Queue Integration (`TCH-014`, `ACD-028`) — Cross-homework & per-homework grading queue API, 2-column institutional grading component (`grading_queue.html`) with instant Ctrl+Enter score & advance and Alt+R return revision.
  - **Milestone 4 (PR #81):** Teacher Mobile Agenda Surface & Substitution Workflow (`ACD-019`, `TCH-001..003`, `TCH-015`) — Substitution workflow with accept/decline actions, decline reason validation, `SUBSTITUTE_DECLINED` notification intent, `DevicePushToken` model & `/api/v1/me/push-tokens/` CRUD, and responsive mobile agenda template (`teacher_agenda.html`).
  - **Milestone 5 (PR #82):** Exam Lockdown Client & Proctor Console (`ACD-024`, `ACD-021..026`) — `GET /exams/:id/proctor/` live monitoring endpoint, `IdempotentViewMixin` for exam autosave, student lockdown screen (`exam_lockdown.html`) with fullscreen enforcement, focus loss counter, autosave, server time resync, and proctor console dashboard (`exam_proctor_console.html`).
