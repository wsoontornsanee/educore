...
- **Current Step:** Milestone M6 (Step 6: Academic Module, P1 Classroom) Done, plus initial Step 6.6 teacher-suite backend slice (agenda + period attendance). Remaining teacher-suite UX (grid, mobile, offline, behaviour, lesson plans, broadcasts) tracked as `[Open Item]` Notion tasks, not yet started.
- **Target Milestone:** P0 Pilot Core [DONE] (M0–M5) -> P1 Classroom (M6–M9): academic module [DONE], teacher suite [backend slice started], report cards [DONE], homework [DONE] (`spec/00 §6`).
