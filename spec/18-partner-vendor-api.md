...
integrators — not scoped to any single domain. It covers three domains from
day one: finance, roster, and attendance. This spec documents the surface a
future implementation task builds against; it does not itself change any
backend code.

| Req ID | Requirement |
|---|---|
| `PVA-001` | The API MUST expose only three domains at launch: `finance`, `roster`, `attendance`. No other internal domain (academic, wallet, campus) is exposed. |
...
| `PVA-011` | Scope (which of the three domains, and which schools within the foundation) is set by the foundation at key-issue time, per domain and per school. A partner cannot self-extend scope. |
...
2. A key holding only `roster.read` gets `403 SCOPE_DENIED` calling a `finance.read`-scoped endpoint.
