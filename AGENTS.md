...
5. **Merge After CI, Then Wait for Deploy Instruction:**
   ...
   - **Review policy (settled 2026-09-19, owner decision).** This is a solo-maintainer repo, so the one required approving review can never be met by anyone but the author, and `--admin` is the routine merge path, not an exception to justify each time. Branch protection deliberately has **no required status checks** and `enforce_admins` is off (owner: no need for required CI checks): the CI gate above is procedural, enforced by this document, not by GitHub. Keep the branch protection as it is. What replaces a second reviewer is: CI green on the current head, conflicts resolved properly under the Pre-PR protocol (never by dropping upstream logic), and, for a change to a money path, an after-the-fact independent review when the owner asks for one (done for #281, #284, #291/#298 on 2026-09-19; findings are Notion items).
