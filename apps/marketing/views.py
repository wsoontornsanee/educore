...
# 'state' drives the template's dot/badge color (operational=success,
# degraded=warning, outage=danger) and is never translated. Static
# illustrative copy — no live uptime monitor exists anywhere in this repo,
# same convention as DASHBOARD_MOCK/APPS/RELEASES above.
STATUS_COMPONENTS = [
    {'name': _('Portal web'), 'state': 'operational'},
    {'name': _('Partner API'), 'state': 'operational'},
    {'name': _('Aplikasi orang tua'), 'state': 'operational'},
    {'name': _('Aplikasi guru'), 'state': 'operational'},
    {'name': _('POS kantin'), 'state': 'operational'},
    {'name': _('Notifikasi (WhatsApp & push)'), 'state': 'operational'},
    {'name': _('Pembayaran (VA & QRIS)'), 'state': 'operational'},
]
...
STATUS_INCIDENTS = [
    {
        'date': _('3 Sep 2026'),
        'severity': 'degraded',
        'title': _('Keterlambatan notifikasi WhatsApp kedatangan'),
        'body': _('Notifikasi kedatangan tertunda hingga 8 menit akibat antrean di penyedia pesan. Diselesaikan dengan menambah kapasitas pengiriman.'),
    },
    {
        'date': _('20 Agu 2026'),
        'severity': 'outage',
        'title': _('Gangguan singkat pada endpoint Partner API'),
        'body': _('/partner/events mengembalikan 503 selama 12 menit akibat migrasi basis data yang tidak otomatis roll back. Diselesaikan dengan roll back manual.'),
    },
]


class StatusView(TemplateView):
    """Public service-status page (static illustrative copy — no real uptime
    monitor exists anywhere in this repo; same convention as ChangelogView
    above, not a live feed)."""
    template_name = 'marketing/status.html'

    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        ctx['active_page'] = 'status'
        ctx['components'] = STATUS_COMPONENTS
        ctx['all_operational'] = all(c['state'] == 'operational' for c in STATUS_COMPONENTS)
        ctx['incidents'] = STATUS_INCIDENTS
        return ctx
